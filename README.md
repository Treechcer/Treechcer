# Hi, I'm Treechcer! 👋

I'm an IT student from Czech with a passion for programming and software development. Currently, I'm focused on projects in Lua, JS and Python, and I'm exploring the basics of game programming.

### 💼 What I Do
- 👨‍💻 Studying IT and working on projects primarily in **Lua**, **JS** and **Python**
- 🎮 Developing simple 2D games using the **Pygame**, **LÖVE2D** (and other) libraries

### 🛠 Technologies
- **Languages**: Java, Python, Lua, JS, HTML, CSS
- **Tools and Frameworks**: Git, [Pygame](https://www.pygame.org/docs/), [LÖVE2D](https://love2d.org)
- **Interests**: Game development, algorithms, and problem-solving

### 📫 Connect with Me
- [GitHub](https://github.com/Treechcer)
- [BlueSky]([https://github.com/Treechcer](https://bsky.app/profile/treechcer.bsky.social))
